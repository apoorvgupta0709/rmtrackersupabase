...
select
  ...
  public.dump_text(r.row -> 18)      as ship_to_party,
...
select
  ...
  public.dump_text(r.row -> 14)      as item_no,
...
select
  ...
  public.dump_text(r.row -> 15)      as credit_control_area,
  public.dump_text(r.row -> 16)      as business_area,
  ...
  public.dump_text(r.row -> 23)      as gl_account,
  public.dump_text(r.row -> 24)      as recon_gl,
  public.dump_text(r.row -> 25)      as offset_account,
...
select
  ...
  public.dump_text(r.row -> 14)      as column1,
...
select
  ...
  public.dump_text(r.row -> 34)      as nrm_po,
  public.dump_text(r.row -> 35)      as cld_po,
  public.dump_text(r.row -> 36)      as finpo,
  public.dump_numeric(r.row -> 37)   as plan_mill,
  public.dump_numeric(r.row -> 38)   as process_grp,
  public.dump_text(r.row -> 39)      as ctg,
  public.dump_text(r.row -> 40)      as spec,
...
select
  ...
  public.dump_text(r.row -> 9)       as length_key,
  public.dump_text(r.row -> 10)      as grade,
  public.dump_text(r.row -> 11)      as fc_nfc,
  public.dump_text(r.row -> 12)      as hr_cr,
  public.dump_numeric(r.row -> 13)   as nos,
  public.dump_text(r.row -> 14)      as c_056,
  public.dump_numeric(r.row -> 15)   as c_0789,
  public.dump_numeric(r.row -> 16)   as c_0788,
  public.dump_numeric(r.row -> 17)   as schedule,
  public.dump_numeric(r.row -> 18)   as stock,
  public.dump_numeric(r.row -> 19)   as in_transit,
  public.dump_numeric(r.row -> 20)   as column2,
  public.dump_numeric(r.row -> 21)   as coverage,
  public.dump_numeric(r.row -> 22)   as c_056_order,
  public.dump_numeric(r.row -> 23)   as c_0789_order,
  public.dump_numeric(r.row -> 24)   as c_0788_order,
  public.dump_numeric(r.row -> 25)   as c_789_order,
  public.dump_numeric(r.row -> 26)   as order_qty_to_be_logged,
  public.dump_numeric(r.row -> 27)   as coverage_post_order,
  public.dump_text(r.row -> 28)      as remark,
  public.dump_text(r.row -> 29)      as unnamed_29
from public.raw_batches b
join public.raw_rows r on r.batch_id = b.id
