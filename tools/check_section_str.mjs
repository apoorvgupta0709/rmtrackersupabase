/**
 * Does the ported STR plan match the pipeline's — rows, waterfall and drill-downs?
 *
 * The plan is what a copy button places as STR lines, so a wrong allocation is a transfer
 * somebody actually raises. Published as a wrapped section, so `data.json` is the oracle.
 *
 *     node tools/check_section_str.mjs /tmp/port/oracle.json --as-of 2026-08-14
 */
...
import { materialDimension, fgCodeForMotherTube } from "../lib/pipeline/sections/material.ts";
...
import { strPlan } from "../lib/pipeline/sections/str.ts";
...
if (!oraclePath) {
  console.error("Usage: node tools/check_section_str.mjs <data.json> [--as-of YYYY-MM-DD]");
  process.exit(2);
}
...
const [bucketting, zmat, oemRows, ledger, scheduleRows, stockRows, rfdRows, wipRows, transferRows, assignmentRows] =
  await Promise.all([
...
const tf = transfers(transferRows, dimension, asOf);

const ours = strPlan(
  resolved, stock, wip, tf, sales, dimension,
  (d, p, b) => fgCodeForMotherTube(dimension, d, p, b),
  tf.plantLabel, asOf,
);
...
for (const [name, mine, theirs] of [
  ["str_plan rows", ours.rows, oracle.str_plan?.rows ?? []],
  ["unmapped_destination_stock", ours.unmappedDestStock, oracle.str_plan?.unmapped_destination_stock ?? []],
  ["unbucketed_schedule", ours.unbucketed, oracle.str_plan?.unbucketed_schedule ?? []],
  ["wip_without_fg_code", ours.wipUnresolved, oracle.str_plan?.wip_without_fg_code ?? []],
]) {
  let bad = 0;
      ...
      for (const f of fields) {
        if (!same(theirs[i][f], mine[i][f])) {
          ...
          if (faults.length < 2000) {
            faults.push(`${name}[${i}].${f} (${theirs[i].bucket ?? theirs[i].customer}): pipeline `
              + `${JSON.stringify(theirs[i][f])?.slice(0, 130)}, port `
              + `${JSON.stringify(mine[i][f])?.slice(0, 130)}`);
          }
        }
      }
    }
...

{
  const theirs = Object.fromEntries(Object.entries(oracle.stock_details ?? {})
    .filter(([k]) => k.startsWith("STRDEST|") || k.startsWith("STRSOURCE|")));
  ...
  summary.push(`  ${String(keys.length).padStart(5)} STRDEST|/STRSOURCE| keys${bad ? `  - ${bad} wrong` : ""}`);
}

console.log(`${groups.length} schedule groups, ${tf.lines.length} transfer lines, `
  + `${ours.rows.length} plan buckets.`);
...
console.log("\nEvery plan bucket, allocation line and drill-down matches the pipeline.");
